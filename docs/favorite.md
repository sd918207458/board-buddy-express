...
上面章節的範例實作並沒什麼問題，也運作得很好，不過也是因為它只是個很簡單的範例，沒考慮到什麼太多的複雜的東西，或是呈現的頁面、資料也很簡單，和大專或實務上的應用可能有些距離。

我們回到最初在"我的最愛"的設計來思考。在呈現商品列表時，"我的最愛"的確是商品資訊的其中一項。另外在會員資料時，它也能是這個會員的資訊中的一個種類，會呈現0對多的情況，也就是說一個會員有可能會有0到多個商品項目，加入到他的"我的最愛"清單裡。

整體的設計是這樣，當會員登入後，才會與愛心圖示的呈現有關，而訪客(非登入會員)的情況時，愛心圖示都是保持空心而且無法操作的，這是我們需要的設計。

所以，我們把有哪些商品加入到這個會員的"我的最愛"，讓它歸屬於這個會員的狀態中，而非和商品狀態歸屬在一起，這個實作一樣可以達成同樣的功能。這只是從另個角度的實作而已，在功能上並不會有太大的差異，但它能改善或改進目前在真正的實作上面臨到的潛在複雜問題(或你已經遇上的)。

我們在會員的登入用的全站Context(勾子)裡的狀態中，加入了一個`favorites`狀態，它代表了這個會員有加入到"我的最愛"的商品id清單:
...
答案並不是說不能直接加入放在一起，加在一起的實作一樣可行。會另外獨立是因為`auth`(或`authJWT`)中的`userData`是單純由會員資料表來的會員登入用的資料，在目前的實作範例中，它在會員登入後，會進入到session或jwt的access token(存取令牌)中一起存放。

這機制的設計會讓新加入`favorites`顯得相當不匹配，尤其是像`favorites`狀態，它是一個會需要經常變動的狀態，它會經常需要新增、刪除，那也代表你每次更新後，也要一併重新更新session或jwt的access token(存取令牌)，在實作上反而變得更麻煩了。`favorites`的確也是一個歸屬於會員的狀態，但它並不合適與session或access token(存取令牌)有關聯到。實作上，除非有這個必要性，我們不會在session或access token(存取令牌)中放入需要經常更新的資料。

> 註: 那會員本身的資料也會更新，是不是也最好別放在session或jwt的access token(存取令牌)中？這一點要視情況而定，會員的資料相較於我的最愛，並不是時常地、這麼容易地作更新的，目前的設計是，會員的資料放在session或access token(存取令牌)是能方便地在伺服器端存取資料用。實務上的確也是不會把會員的所有資料放在裡面，尤其是會常更動的會員資料，大概只會放uid、username的就行了(不能更動的會員資料等等)，在目前提供的範例並沒有區分到這麼細部。
...
所以商品的sql與伺服器的實作，它不再需要透過會員認証的middleware。它就和原本不論是訪客或會員都能看到商品資訊一樣而已，這部份使用你如果原本有的商品列表機制就行了，它也不需要什麼我的最愛的欄位，我的最愛的呈現將會不由商品的資料來決定。程式碼範例如下:
